...
public:
	...
	void set_flip(bool flip_h)
	{
		this->is_flip_h = flip_h;
	}
	...
	void on_render(SDL_Renderer* renderer) const
	{
		...
		rect_dst.y = position.y - frame.rect_src.h;
		rect_dst.w = (float)frame.rect_src.w, rect_dst.h = (float)frame.rect_src.h;

		SDL_RenderCopyExF(renderer, frame.texture, &frame.rect_src, &rect_dst, angle, &center, is_flip_h ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
	}
