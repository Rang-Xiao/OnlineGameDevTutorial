...
玩家真正感受到的是"按下按键到画面变化"的总时长，网络只是其中一段：
...
关键认知：**本地环节加起来常常和网络一样大**。玩家抱怨"卡"时，未必是网络的锅 —— 垂直同步 + 三重缓冲能凭空加 50 ms，比跨省网络还狠。

**非网络延迟因素**

- 输入采样延迟
- 渲染流水线延迟：显卡会把绘制命令塞入缓冲区，如果渲染负担重，GPU会有延时
- 垂直同步/显示延迟：为了放置画面撕裂，GPU把更新频率降到与显示器帧率一致

补充两个常被忽略的本地环节：

- **逻辑帧与网络帧不对齐**：输入在两次发包之间产生，平均要多等半个发包间隔。20 Hz 发包 = 平均多 25 ms
- **服务端 tick 对齐**：包到达时刚好错过本 tick，要等下一 tick 才被处理，同样平均半个 tick

> 这两个"半帧"是纯设计代价，和网络质量无关。提高 tick rate 能减少它们，代价是带宽。

**网络延迟因素**

- 处理延迟：取决于路由器，一般很快
- 传输延迟：把数据变成二进制流灌入物理介质所花的事件。取决于用户终端的网络类型和带宽
- 排队延迟：少量大包优与大量小包
- 传播延迟：架设更近的服务器

| 成因 | 典型量级 | 决定因素 | 解决方法 |
|------|----------|----------|----------|
| 处理延迟 | μs 级 | 路由器性能 | 基本无法干预 |
| 传输延迟 | 包大小 / 带宽 | 上行带宽 | 减小包体（序列化选型、delta 压缩） |
| 排队延迟 | 0~数百 ms，**波动最大** | 链路拥塞程度 | 少发大包、控制发送频率、避免突发 |
| 传播延迟 | 距离 / 光速，**硬下限** | 物理距离 | 只能把服务器搬近（分区服、边缘节点） |

"少量大包优于大量小包"的原因：每个包都要付固定开销：IP 头 20 字节 + UDP 头 8 字节（TCP 是 20），以及路由器每包一次的处理成本。发 10 个 20 字节的包，光包头就 280 字节，有效载荷占比不到 42%；合成 1 个 200 字节的包，包头只有 28 字节。

所以游戏服务端普遍**按 tick 合批**：一个 tick 内所有要发给同一玩家的消息打成一个包发出，而不是产生一条发一条。
...
抖动会影响RTT，从而影响基于RTT的预测补偿算法

更有可能导致包乱序到达

**抖动的定义**

抖动（Jitter）是**延迟的方差**，不是延迟本身。稳定的 150 ms 比在 30~120 ms 之间反复横跳的链路好玩得多：

```
低抖动（平均 80ms）        高抖动（平均 80ms）
包1 ──80──→               包1 ──30──→
包2 ──82──→               包2 ─────140──→   ← 后发先至，乱序
包3 ──79──→               包3 ──35──→
包4 ──81──→               包4 ────110──→
手感：稳定可预测            手感：忽快忽慢，人物瞬移
```

原因：人脑能适应恒定延迟（提前量打出去就行），却无法适应随机延迟。**恒定的坏比随机的好更好。**

**成因**

- 处理延迟：占比较小，路由器动态调整数据包路线
- 传输延迟/传播延迟：路由器动态进行负载均衡
- 排队延迟：突发网络流量导致路由器排队数增加

排队延迟是抖动的**主要来源**：路由器队列的长短随全网流量实时变化，你的包可能一次直接通过、下一次排在几百个包后面。家里有人开始下载/看视频，游戏立刻抖，就是这个原因。

**解决方案**

使用可靠的传输协议。在应用层解决乱序问题

具体手段有三层：

| 手段 | 做法 | 代价 |
|------|------|------|
| **序号 + 重排** | 每包带递增序号，收端按序号排序后再交给逻辑层 | 需要缓存，实现简单 |
| **抖动缓冲（Jitter Buffer）** | 收到的包不立即用，先缓冲 1~3 个 tick 再按节奏播放 | **主动增加固定延迟**，换取平滑 |
| **丢弃过期包** | 位置类状态包，旧的直接丢，只用最新的 | 无代价，但仅适用于"可覆盖"的数据 |

抖动缓冲是这里最反直觉的一招：**故意让自己更慢，来换取不抖**。客户端插值（世界状态同步一节）本质就是一个抖动缓冲 —— 永远渲染 100 ms 前的状态，从而总有两个快照可供插值。

> 序号的另一个作用：识别**重复包**。UDP 下重传可能造成同一个包收到两次，靠序号去重。
...
- 不可靠的物理介质
- 不可靠的链路层：当数据损坏，链路层通过校验和发现后，就会直接丢弃这一帧数据。信道占满时，新来的数据帧不得不丢弃
- 不可靠的网络层：缓存队列满了，路由器只能丢掉部分包

补充一个游戏里最常见却最易忽视的成因：**自己打满了上行**。客户端每帧发大包、服务端不限频广播，把家用宽带上行塞满后，路由器队列溢出开始丢包 —— 这是**可以靠优化协议直接消除**的丢包，而前三种不能。
...
|--------|------|
| < 1% | 基本无感，重传/插值可完全掩盖 |
| 1~5% | FPS 出现瞬移、判定失准，MMO 尚可玩 |
| > 5% | 任何补偿都掩盖不住，体感崩溃 |

注意丢包对 **TCP** 的伤害是非线性的：TCP 把丢包解读为"网络拥塞"，会主动降低发送速率，1% 的丢包可能让吞吐掉一半以上。

**解决方案**

使用可靠的传输协议

但"可靠"分两种，选错了比不可靠更糟：一种是TCP的阻塞等待，一种是可靠UDP的非阻塞等待

**TCP 的队头阻塞（Head-of-Line Blocking）**

TCP 保证**按序交付**，于是一个包丢了，后面已经到达的包全都得在内核缓冲区里干等：

```
发送：[1][2][3][4][5]
       ↓  ✗  ↓  ↓  ↓        包2 丢失
到达：[1]   [3][4][5]
应用收到：[1] ...... 等待 2 重传 ...... [2][3][4][5]
                ↑
        3、4、5 明明已经到了，却被卡住 —— 一个 RTT 的停顿
```

对游戏的致命之处：位置同步包 3、4、5 里已经有**更新的位置**了，等来的包 2 却是**过期数据**。玩家看到的是画面卡住一下，然后瞬移到新位置。

这正是**游戏普遍选 UDP** 的根本原因 —— 不是 UDP 更快，而是 UDP 不强制排队等待，应用层可以自己决定"哪些数据值得重传，哪些丢了就算了"。

**按数据类型选择可靠性**

| 数据类型 | 例子 | 可靠性要求 | 丢了怎么办 |
|----------|------|-----------|-----------|
| **状态快照** | 位置、朝向、血量 | 不可靠 | 直接丢，下一帧的更新更有价值 |
| **事件 / RPC** | 开火、技能、买道具 | 可靠有序 | 必须重传，丢了逻辑就错了 |
| **生命周期** | spawn / destroy | 可靠 | 必须重传，否则客户端对象表错乱 |
| **聊天、结算** | 文本、奖励 | 可靠，但不急 | 重传，可低优先级 |

这张表就是 ENet「多通道 + 每包独立可靠性标志」设计的由来：同一条连接上，位置走不可靠通道，RPC 走 `ENET_PACKET_FLAG_RELIABLE`（Demo_5/7 用的正是这个标志）。

#### 总结

| 干扰 | 本质 | 主要对策 | 代价 |
|------|------|----------|------|
| 延迟 | 慢 | 客户端预测、延迟补偿、就近部署 | 需要和解回滚，实现复杂 |
| 抖动 | 不稳 | 序号重排、抖动缓冲、插值 | **主动增加固定延迟** |
| 丢包 | 缺 | 选择性重传、冗余、状态可覆盖 | 带宽或延迟二选一 |

> 注意最后一列：三种对策的代价互相打架 —— 抖动缓冲加延迟，重传也加延迟，而降低延迟又要求少缓冲少重传。**联机优化的全部艺术就在于按游戏类型决定这个平衡点**：FPS 优先低延迟（宁可丢包、用预测硬扛），MMO 优先平滑（可以多缓冲 100 ms 没人在意）。
...
```
应用层           你的 RPC / 状态同步        ← 两者都要你自己写
─────────────────────────────────────
"会话层"       ENet 协议（固定 wire format）
               连接、通道、可靠性
─────────────────────────────────────
传输层         TCP  │  UDP              ← SDL_net 就封到这一层为止
─────────────────────────────────────
网络层         IP                        ← 谁都碰不到
```

在UDP上建立一个可靠、鲁棒、可定制的易用网络层协议

会话层：ENet / UTP / KCP / LiteNetLib(C#) / GameNetworkingSockets(Valve) / QUIC ，都是对传输层一定程度的封装

应用层： Mirror / Fish-Net / Photon PUN / NGO ，它们底下往往可换 transport（Mirror 就有 KCP、Telepathy、ENet 等多种 transport 插件）

**优势**

- 保序Sequencing，无论是否可靠，都保证排序
- Channel不同可靠性数据放到不同频道
- 数据包的分片和重组：会自动把大数据分成小块，再拼成完整的
- 轻量可移植
...
策略类游戏允许较大的延时，甚至用http都行

#### 状态同步

**导航推测法 Dead Reckoing** 

是一种乐观算法，一种对侧客户端预测，即一个客户端预测另一个客户端的运动模式
...
- 相位滞后：始终让影子滞后于最新状态以方便调整
- 惯性移动：运动状态急剧变化时重置速度并使用加速度启动

### 竞技游戏优化策略（帧同步）

#### 帧同步步骤

- 客户端在每个逻辑帧向服务器报告自己的状态或操作
- 服务器收到所有客户端上传的当前关键帧数据后运算并将结果返回给所有客户端
- 如果客户端在下一个关键帧到来之前还未收到数据则**帧锁定**，进入到等待阶段
- 收到服务器返回后根据数据进行渲染，并记录状态和操作重复第一步

**乐观锁与普通帧同步差异**

用于防止一个人数据传不上来导致全员干等的情况

- 服务器将游戏时间划分为多个逻辑帧回合
- 每帧结束时如果没有收到某位玩家行动则认为他放弃了本帧操作不再等待
- 如果网络较慢的玩家上传的操作明显超时，则根据需求选择丢弃或者其他处理

### AOI思想详解：游戏同步数据优化与区域管理算法
...
对场景中所有坐标进行x方向y方向排序（有点空间换时间的感觉）

粗筛出一个长方形，表明是需要做aoi的区域，从而避免对所有玩家进行遍历

之后可以再用视锥精筛

#### 处理AOI：观察者&被观察者

观察者和被观察者持有双方的集合引用，从而避免粗暴遍历
...
劣势：大部分文本格式不支持增量读写。这会造成序列化开销

- 关系数据库

优势：便于统计，支持权限管理容灾机制，支持增量更新

劣势：树级表格复杂，需要加载嵌套的多个表到内存
...
问题：容灾能力差，分散存盘可能引发逻辑问题

- 提高存盘效率
- 每次重要操作写入服务器日志，有问题时可以回溯日志核验，或者进行增量更新
...
为每个资源包简历一个缩影表，key是文件相对路径的哈希值，value是文件在包里的位置偏移和大小

这样可以通过文件路径索引到文件大小

对于二进制文件，一般体积较小可以直接覆盖

**游戏包体增删操作**

- 删除：仅需删除包头索引表
- 新增：检查是否有闲置空间，否则追加到包体尾部
- 修改：若变更体积小则直接覆盖，否则执行先删除后新增

#### 热更新

静态语言可以靠反射来动态链接

动态语言由于边解释边加载的特性，天然支持热更新

### 调试技术详解：联机游戏问题排查与日志系统规范

#### 日志

- 用途：时间序列化、状态快照、与服务器日志对齐
- 优点：轻量、可持久化、可上报，易于跨段关联；能长期保留在用户端
- 局限：写太多会影响性能并且改变时序；单靠日志不足以还原内存状态；海量日志需要后端处理能力
...
- 用途：本机原生崩溃时抓取进程快照
- 优点：能直接定位C++崩溃位置，对内存访问违例/异常很有用
- 局限：需要符号PDB才有价值；不一定能显示脚本VM的高度调度状态
- 使用场景：硬崩溃/原生死锁/OOM

### Profiler / Tracer

- 用途：性能剖析、帧事件分析、锁/等待热点、GPU/CPU负载
- 优点：适合在开发期定位问题
- 局限：通常开销较大，不适合在用户机上运行
- 使用场景：性能优化、卡顿复现、锁竞争诊断
