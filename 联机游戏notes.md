...
> 联机游戏的三大敌人：延迟（慢）、抖动（不稳）、丢包（缺）。它们决定了前两章所有同步方案的实际手感

先明确一个前提：**这三者都不可能被消除，只能被隐藏**。光速是物理上限，路由器队列必然存在。所有优化的本质都是"用预测、缓冲、插值把不确定性藏到玩家看不见的地方"。
...
光纤中信号约 20 万 km/s，且实际路由绕路通常是直线距离的 1.5~2 倍：

```
北京 ↔ 上海   ~1100 km  →  单向 ≈ 8 ms，RTT ≈ 16 ms（实测常 30 ms）
中国 ↔ 美西   ~10000 km →  单向 ≈ 70 ms，RTT ≈ 140 ms（实测常 200 ms+）
```

这解释了为什么竞技游戏必须**分区服**：跨洲对战的 RTT 光物理层就吃掉了 FPS 能容忍的全部预算。

**带宽 ≠ 延迟**

常见误解是"网速快就不卡"。千兆宽带和百兆宽带 ping 同一台服务器，延迟几乎一样 —— 带宽决定**一次能塞多少**，延迟决定**第一个字节多久到**。游戏包普遍只有几十到几百字节，带宽从来不是瓶颈，**延迟和稳定性才是**。

**"少量大包优于大量小包"的原因**

每个包都要付固定开销：IP 头 20 字节 + UDP 头 8 字节（TCP 是 20），以及路由器每包一次的处理成本。发 10 个 20 字节的包，光包头就 280 字节，有效载荷占比不到 42%；合成 1 个 200 字节的包，包头只有 28 字节。
...
上下行路由可能不同，拥塞程度也常不对称（家用宽带上行普遍远小于下行）。所以 `单向 = RTT / 2` 只是估算。要精确测单向延迟必须做**时钟同步**，代价是两端时钟本身也在漂移 —— 游戏里通常懒得做，直接用 RTT/2 凑合。
